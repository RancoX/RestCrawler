...
class RestCrawler:
    ...
    def __init__(self, folder_name='Demo', start_date='31/01/2014', months_under_analysis=36, funds=['Core Strategy', ], debug=True):
        ...
        print(f"The analysis will be done for the period {self.start_date} ~ {(datetime.strptime(self.start_date,r'%d/%m/%Y') + relativedelta(months=+(months_under_analysis-1),day=31)).strftime(r'%d/%m/%Y')}")
    ...
    def create_dataframe(self):
        ...
        self.df.set_axis(['Date', 'Option', '10yr', '7yr', '5yr','3yr', '1yr', '6m', '3m', 'FYTD'], axis=1, inplace=True)

        # 3.2 format dates and reorder

        self.df.Date = self.df.Date.apply(datetime.strptime, args=('%Y_%b_%d',))

        self.df.sort_values(by='Date', inplace=True)

        self.df.Date = self.df.Date.apply(datetime.strftime, args=('%d-%m-%Y',))
    ...
    def line_chart(self):
        ...
        self.df=self.df.astype({'10yr':'float','7yr':'float','5yr':'float','3yr':'float','1yr':'float','6m':'float','3m':'float','FYTD':'float'})
        ...
        app.layout=html.Div([
            ...
            html.Div([
                ...
                dcc.Dropdown(id='fund_selector',options=self._funds,value=self._funds[1])
            ...
            html.Div([
                ...
                dcc.Graph(id='line_chart',hoverData={'points':[{'customdata':'Core Strategy'}]})
        ...
        def update_linechart(fund,year):
            ...
            dff=self.df[(self.df['Option']==fund) & (self.df['Year']==year)]
            ...
            fig.add_trace(go.Scatter(x=dff['Date'],y=dff['5yr'],mode='lines',line={'color':'gray','width':1,'dash':'dash'}))
...
if __name__ == '__main__':
    ...
    options = ['Core Strategy', 'Balanced', 'Balanced - Indexed', 'Capital Stable', 'Diversified', 'High Growth', 'Cash', 'Bonds', 'Shares', 'Property',

               'Australian Shares', 'Australian Shares - Indexed', 'Overseas Shares', 'Overseas Shares - Indexed']
